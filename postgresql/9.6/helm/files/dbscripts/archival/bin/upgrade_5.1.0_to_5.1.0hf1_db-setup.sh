...
psql -U "${pg_archival_user}" -d "${pg_archival_database}" -h "${PG_HOST}" -p "${PG_PORT}" -v pg_tablespace="${pg_archival_tablespace}" -f $SCRIPTPATH/upgrade_5.1.0_to_5.1.0hf1_ddl.sql
