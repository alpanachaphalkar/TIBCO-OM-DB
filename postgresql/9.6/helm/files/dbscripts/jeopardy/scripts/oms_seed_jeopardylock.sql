create or replace FUNCTION OMS_Seed_JeopardyLock( IN EXPONENT int, IN tenant_id character varying) returns void
LANGUAGE plpgsql
AS	$$
DECLARE
...
BEGIN
	...
	delete from JEOPARDY_LOCK where TENANTID = tenant_id;
	raise notice'Removed old entries for the hash indexes';

	  v_rowcount := POWER( 2, EXPONENT );
	  FOR i in 0 .. v_rowcount-1 LOOP
		   
		   insert into JEOPARDY_LOCK(HASHINDEX,TENANTID) values (i,tenant_id);
...
EXCEPTION WHEN OTHERS THEN
  	raise notice 'An error occured while seeding jeopardy lock';
